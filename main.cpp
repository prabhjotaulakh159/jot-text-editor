...
void SaveFile(const HWND& editArea, const std::wstring& filename, FileLoader& fileLoader) {
  auto len = GetWindowTextLength(editArea);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  ...
  auto *appState = new (std::nothrow) AppState;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  switch(uMsg) { 
    case WM_CREATE: {
      ...
      appState = reinterpret_cast<AppState*>(pCreate->lpCreateParams);
      SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR)appState);

      menu = SetupBasicMenu();
      editArea = SetupInputArea(hwnd);
      SetMenu(hwnd, menu);
      ShowWelcomeMessage(editArea);
      return 0;
    }
    ...
    case WM_COMMAND: {
      ...
      if (UserChoosesOpen(wParam)) {
        auto newCurrentFile = AccessFileExplorer(hwnd, state->currentFile, JOT_TEXT_EDITOR_MENU_OPEN_ID);
        if (newCurrentFile) {
          state->currentFile = newCurrentFile.value(); 
          fileLoader.readFileIntoLines(state->currentFile); 
          OutputFileContentOnEditArea(editArea, fileLoader.getLines());
        }
      } 
      else if (UserChoosesSave(wParam)) {
        auto newCurrentFile = AccessFileExplorer(hwnd, state->currentFile, JOT_TEXT_EDITOR_MENU_SAVE_ID);
        if (newCurrentFile) {
          state->currentFile = newCurrentFile.value(); 
          SaveFile(editArea, state->currentFile, fileLoader);
        }
      } 
      else if (UserChoosesExit(wParam)) {
        if (UserWantsToSaveBeforeQuitting(hwnd)) {
          auto newCurrentFile = AccessFileExplorer(hwnd, state->currentFile, JOT_TEXT_EDITOR_MENU_SAVE_ID);
          if (newCurrentFile) {
            state->currentFile = newCurrentFile.value(); 
            SaveFile(editArea, state->currentFile, fileLoader);
